package deeplearning4j.smile.code_examples.RecommendationSystem
...
object RecommendationSystem extends SimpleSwingApplication {
  ...
  def top = new MainFrame {
    ...
    val basePath = "subprojects/machinelearning/src/main/scala/smile/example_data"
  ...
  def getDateFromEmail(email: String): Date = {
    ...
    datePatterns.foreach { x =>
      ...
      Try(return new SimpleDateFormat(x, Locale.ENGLISH).parse(email.substring(dateLineIndex + 5, endOfDateLine).trim.substring(0, x.length)))
    }
  ...
  def getStopWords: List[String] = {
    val source = scala.io.Source.fromFile(new File("subprojects/machinelearning/src/main/scala/smile/example_data/stopwords.txt"))("latin1")
